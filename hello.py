...
def main():
    ...
    st.write(
        "ベロシティとスコープクリープから終了時期をモンテカルロシミュレーションします"
    )

    # シミュレーションパラメータ
    st.header("ストーリーポイント")
    story_point = st.slider(
        "累計ストーリーポイント", min_value=100, max_value=500, value=200, step=10
    )

    st.header("チーム")
    velocity_mean = st.number_input(
        "ベロシティの平均値", min_value=10.0, max_value=200.0, value=30.0, step=10.0
    )

    velocity_std_dev = st.number_input(
        "ベロシティの標準偏差", min_value=1.0, max_value=50.0, value=10.0, step=1.0
    )

    st.header("スコープクリープ")
    st.write(
        "スコープクリープとは、プロジェクトの進行中に要件が増加することを表します。"
    )
    st.write("各スプリントごとにどれだけ増加するかを表します。")
    ...
    scope_creep_std_dev = st.number_input(
        "スコープクリープの標準偏差 (%/sprint)",
        min_value=0.0,
        max_value=5.0,
        value=1.0,
        step=0.1,
    ...
    sprint_duration = st.number_input(
        "スプリント期間 (日)", min_value=1, max_value=50, value=14, step=1
    )
...
def monte_carlo_simulation(
    ...
    Args:
        story_point (float): Total story points to complete.
        ...
        while total_tasks > 0:
            ...
            if total_tasks <= velocity_per_sprint:
                ...
                creep_rate = np.random.normal(
                    scope_creep_mean / 100, scope_creep_std_dev / 100
                )
